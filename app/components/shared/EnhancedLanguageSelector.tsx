...
import { getPreferredLanguage, setLanguageCookie } from '@/app/utils/languageDetection';
...
export default function EnhancedLanguageSelector({
  ...
  useEffect(() => {
    const detectAndSetLanguage = async () => {
      try {
        ...
        if (currentLocale === 'en' && !document.cookie.includes('NEXT_LOCALE=')) {
  ...
  }, [isOpen, languages.length, closeDropdown, focusNextLanguage, focusLanguageByIndex]);
  ...
  const getDropdownStyles = () => {
    const baseStyles = 'absolute z-50 bg-white/95 rounded-2xl shadow-lg ring-1 ring-black ring-opacity-5 overflow-hidden border border-gray-100 backdrop-blur-sm';
